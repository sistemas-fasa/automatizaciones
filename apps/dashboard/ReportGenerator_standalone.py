...
class ReportGenerator:
    ...
    def load_and_encode_js(self, filename):
        """Load JavaScript file and return it for embedding"""
        try:
    ...
    def generate_tailwind_table(self, title, data_dict, total_dia):
        ...
        for detalle, monto in data_dict.items():
            ...
            if bar_width >= 70:
                ...
                gradient_class = "yellow"
            else:
                gradient_class = "red"
            row = f"""<tr><td>{detalle}</td><td>{self.format_currency(total_dia_val)}</td><td style="text-align:right">{porcentaje:.1f}%</td><td>{self.format_currency(total_mes_val)}</td><td><div class="progress-bar"><div class="progress-fill {gradient_class}" style="width: {bar_width:.1f}%"></div><span class="progress-text">{porcentaje:.1f}%</span></div></td></tr>"""
            rows.append(row)
        
        html_table = f"""<table>
<div class="table-header"><h3>{title}</h3></div>
<thead><tr><th>Detalle</th><th>Monto</th><th>%Dia</th><th>Total mes</th><th>Visual</th></tr></thead>
<tbody>{"".join(rows)}</tbody>
    ...
    def generate_html_report(self, stats, monthly_total, fecha):
        ...
        # Load JS libraries
        chartjs_code = self.load_and_encode_js('chart.min.js')
        datalabels_code = self.load_and_encode_js('chartjs-plugin-datalabels.min.js')
        
        # Build HTML in parts to avoid f-string issues with JS code
...
"""
        ...
        html_chartjs = f"<script>\n{chartjs_code}\n</script>\n"
        
        # Add DataLabels plugin
        html_datalabels = f"<script>\n{datalabels_code}\n</script>\n"
...
new Chart(document.getElementById('agingChart'), {{
    ...
    options: {{
        ...
        # Combine all parts
        html = html_header + html_chartjs + html_datalabels + html_content + html_charts_script
        return html
